...
def test_no_false_positive_on_publish_discussion():
    """Regression: 'publication' / 'publish' in synthesis about a human
    downstream step must not block when only producing a review document."""
    synth = (
        "Decision Point: Operator produces a draft board packet for director "
        "review. The Director fills placeholders before publication. "
        "Confidence: High"
    )
    check("publication mention without operator transmit not blocked",
          g(synth, EDU) is None)
...
def main():
    ...
    for t in [test_gate_off_when_no_bounds,
              ...
              test_no_false_positive_on_publish_discussion,
